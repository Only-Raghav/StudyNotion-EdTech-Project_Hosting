...
exports.editCourse = async (req, res) => {
  ...
  if (Object.prototype.hasOwnProperty.call(updates, key)) {
    if (key === 'tag' || key === 'instructions') {
      ...
      try {
        // Only try to parse if the value is a string and appears to be a valid JSON
        if (typeof updates[key] === 'string' && updates[key].trim().startsWith('[') || updates[key].trim().startsWith('{')) {
          course[key] = JSON.parse(updates[key]);
        } else {
          // If it's not a valid JSON, just assign the value
