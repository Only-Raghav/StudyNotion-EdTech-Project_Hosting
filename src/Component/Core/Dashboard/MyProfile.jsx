      ...
      <div className='flex flex-col gap-y-5 md:gap-y-10 rounded-md border border-richblack-700 p-8 px-3 md:px-12 bg-richblack-800 '>
        ...
        <div className='flex flex-col md:flex-row gap-y-5'>
          ...
          <div className='w-full md:w-1/2'>
            ...
            <p className={`text-sm font-medium ${user?.additionalDetails?.dateOfBirth ? 'text-richblack-5' : 'text-richblack-400'} `} >{user?.additionalDetails?.dateOfBirth ? formattedDate(user?.profile?.dob) : 'Add Date of Birth'}</p>
          </div>
        </div>
      </div>
